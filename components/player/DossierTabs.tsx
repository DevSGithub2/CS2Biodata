  ...
  return (
    <div className="space-y-6 font-mono">
      ...
      <div className="min-h-[440px]">
        ...
        {activeTab === "faceit" && (
          <div className="rounded-lg bg-[#0d1117]/95 border border-white/[0.08] overflow-hidden shadow-[0_8px_32px_rgba(0,0,0,0.7)] font-mono">
            {loadingFaceit ? (
              <div className="py-28 flex flex-col items-center justify-center gap-3 text-cyan-400">
                ...
                <span className="text-xs tracking-widest uppercase text-gray-400">
                  Syncing official FACEIT profile &amp; match history...
                </span>
              </div>
            ) : !faceitIntel && !data?.faceit?.elo ? (
              <div className="py-24 text-center text-gray-500 text-xs">
                No active FACEIT profile tied to this Steam account.
              </div>
            ) : (
              <div>
                {/* 1. Official FACEIT Cover Banner */}
                <div className="relative w-full h-44 sm:h-56 bg-[#161b22] overflow-hidden">
                  {faceitIntel?.coverImage ? (
                    <img src={faceitIntel.coverImage} alt="Cover" className="w-full h-full object-cover opacity-70" />
                  ) : (
                    <div className="absolute inset-0 bg-gradient-to-r from-[#FF5500]/35 via-[#161b22] to-black" />
                  )}
                  ...
                  <div className="absolute top-4 right-4 flex items-center gap-2.5 z-20">
                    <div className="px-4 py-2 rounded-md bg-[#161b22]/95 backdrop-blur-md border border-[#FF5500]/60 text-white font-mono text-xs font-black flex items-center gap-3 shadow-2xl">
                      {(() => {
                        const badge = { bg: "#FF5500", text: "#000000", label: String(faceitIntel?.skillLevel || data?.faceit?.skillLevel || 8) };
                        return (
                          <div className="w-6 h-6 rounded flex items-center justify-center font-black text-xs text-black shadow" style={{ backgroundColor: badge.bg }}>
                            {badge.label}
                          </div>
                        );
                      })()}
                      <span className="text-gray-400 font-bold">ELO</span>
                      <span className="text-[#FF5500] text-sm font-black">{faceitIntel?.elo || data?.faceit?.elo || "—"}</span>
                    </div>

                    {faceitIntel?.faceitUrl && (
                      <a
                        href={faceitIntel.faceitUrl}
                        target="_blank"
                        rel="noopener noreferrer"
                        className="flex items-center gap-1.5 px-4 py-2 rounded-md bg-[#FF5500] hover:bg-[#e04b00] text-black text-xs font-black uppercase tracking-wider transition-all shadow-md"
                      >
                ...
                {/* 2. Avatar & Bio Bar */}
                <div className="px-6 pb-6 relative -mt-16 sm:-mt-20 flex flex-col md:flex-row md:items-end justify-between gap-4 border-b border-white/[0.08]">
                  <div className="flex items-end gap-4">
                    <div className="relative w-24 h-24 sm:w-28 sm:h-28 rounded-xl bg-[#161b22] border-2 border-[#FF5500] overflow-hidden shadow-[0_0_25px_rgba(255,85,0,0.4)] shrink-0">
                      {faceitIntel?.avatar ? (
                        <img src={faceitIntel.avatar} alt="Avatar" className="w-full h-full object-cover" />
                      ) : (
                        <div className="w-full h-full flex items-center justify-center text-2xl font-bold text-[#FF5500]">
                          {faceitIntel?.nickname?.[0] || "F"}
                        </div>
                      )}
                      <div className="absolute bottom-1 right-1 px-1.5 py-0.5 rounded bg-[#FF5500] border border-black font-black text-[10px] text-black shadow-md">
                        {faceitIntel?.skillLevel || data?.faceit?.skillLevel || "8"}
                      </div>
                    </div>

                    <div className="space-y-1 mb-1">
                      <div className="flex items-center gap-2.5">
                        <h2 className="text-xl sm:text-2xl font-black text-white uppercase tracking-wider drop-shadow">
                          {faceitIntel?.nickname || "FACEIT Operative"}
                        </h2>
                        {faceitIntel?.country && (
                          <span className="px-2.5 py-0.5 rounded bg-white/[0.06] border border-white/[0.1] text-[10px] text-gray-300 font-mono uppercase">
                            📍 {faceitIntel.country}
                          </span>
                        )}
                        <span className="px-2.5 py-0.5 rounded bg-[#FF5500]/10 border border-[#FF5500]/40 text-[#FF5500] text-[10px] font-bold uppercase">
                          {faceitIntel?.membershipType || "FREE"}
                        </span>
                      </div>
                      <p className="text-xs text-gray-400 italic max-w-lg">
                        &ldquo;{faceitIntel?.bio || "Competitive Counter-Strike 2 Operative"}&rdquo;
                      </p>
                    </div>
                  </div>
                </div>
                ...
                <div className="p-6 space-y-6">
                  ...
                  <div>
                    ...
                    <div className="grid grid-cols-2 sm:grid-cols-4 gap-3.5">
                      <div className="p-4 rounded-lg bg-[#161b22] border border-white/[0.08]">
                        <span className="text-[10px] text-gray-400 uppercase font-bold">Total Matches</span>
                        <div className="text-2xl font-black text-white mt-2 font-mono">
                          {faceitIntel?.lifetime?.matches || "—"}
                        </div>
                      </div>
                      ...
                      <div className="p-4 rounded-lg bg-[#161b22] border border-white/[0.08]">
                        ...
                        <div className="text-2xl font-black text-emerald-400 mt-2 font-mono">
                          {faceitIntel?.lifetime?.winRate ? `${faceitIntel.lifetime.winRate}%` : "—"}
                        </div>
                      </div>

                      <div className="p-4 rounded-lg bg-[#161b22] border border-white/[0.08]">
                        <span className="text-[10px] text-gray-400 uppercase font-bold">Average K/D</span>
                        <div className="text-2xl font-black text-white mt-2 font-mono">
                          {faceitIntel?.lifetime?.kdRatio || "—"}
                        </div>
                      </div>

                      <div className="p-4 rounded-lg bg-[#161b22] border border-white/[0.08]">
                        <span className="text-[10px] text-gray-400 uppercase font-bold">Headshot Ratio</span>
                        <div className="text-2xl font-black text-cyan-400 mt-2 font-mono">
                          {faceitIntel?.lifetime?.headshots ? (Number(faceitIntel.lifetime.headshots) > 100 ? `${(Number(faceitIntel.lifetime.headshots)/100).toFixed(0)}%` : `${faceitIntel.lifetime.headshots}%`) : "—"}
                        </div>
                      </div>
                    </div>
                  </div>

                  {/* Official FACEIT Match History Table with Map Icons */}
                  <div className="pt-2">
                    <div className="flex items-center justify-between mb-3">
                      <span className="text-[11px] font-bold text-gray-400 uppercase tracking-widest">
                        RECENT MATCH HISTORY
                      </span>
                      <span className="text-[10px] text-[#FF5500] font-bold uppercase">Official Matchmaking Logs</span>
                    </div>

                    {faceitIntel?.matches && faceitIntel.matches.length > 0 ? (
                      <div className="rounded-lg bg-[#161b22] border border-white/[0.08] overflow-hidden shadow-md">
                        <div className="grid grid-cols-12 px-5 py-3.5 bg-black/50 border-b border-white/[0.08] text-[11px] font-bold text-gray-400 uppercase tracking-wider">
                          ...
                          <span className="col-span-3 text-right">Map &amp; Mode</span>
                        </div>

                        <div className="divide-y divide-white/[0.04]">
                          {faceitIntel.matches.map((m: any, idx: number) => {
                            const isWin = m.result === "W";
                            const mapMeta = getOfficialMapAsset(m.map);

                            return (
                              <div key={idx} className="grid grid-cols-12 px-5 py-3.5 items-center text-xs hover:bg-white/[0.03] transition-colors">
                                <span className="col-span-3 text-gray-300 font-mono text-xs">{m.date}</span>
                                <span className="col-span-2 text-center font-black font-mono text-white text-sm">{m.score}</span>
                                <div className="col-span-2 flex justify-center">
                                  <span className={`px-3 py-1 rounded text-[10px] font-black uppercase tracking-widest ${
                                    isWin 
                                      ? "bg-emerald-950/90 border border-emerald-500/50 text-emerald-400 shadow-[0_0_8px_rgba(16,185,129,0.2)]" 
                                      : "bg-rose-950/90 border border-rose-500/50 text-rose-400 shadow-[0_0_8px_rgba(244,63,94,0.2)]"
                                  }`}>
                                    {isWin ? "WIN" : "LOSS"}
                                  </span>
                                </div>
                                <span className="col-span-2 text-center font-mono text-gray-200 text-xs font-bold">{m.kdA}</span>
                                <div className="col-span-3 flex items-center justify-end gap-2.5">
                                  <span className="font-black text-white uppercase tracking-wide">{mapMeta.name}</span>
                                </div>
                              </div>
                            );
                          })}
                      ...
                      <div className="p-8 text-center text-gray-500 text-xs rounded bg-[#161b22] border border-white/[0.08]">
                        No recent match history found in the API buffer.
                      </div>
                    )}
                  </div>

                  {/* Map Telemetry Breakdown with Official Map Icons */}
                  {faceitIntel?.mapStats && faceitIntel.mapStats.length > 0 && (
                    <div className="pt-2">
                      <span className="text-[11px] font-bold text-gray-400 uppercase tracking-widest block mb-3">
                        OFFICIAL MAP TELEMETRY BREAKDOWN ({faceitIntel.mapStats.length} MAPS)
                      </span>

                      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-3.5">
                        {faceitIntel.mapStats.map((m: any, idx: number) => {
                          const mapMeta = getOfficialMapAsset(m.map || m.label || "");
                          const winRateNum = parseFloat(m.winRate) || 0;

                          return (
                            <div
                              key={idx}
                              className="relative group rounded-lg overflow-hidden border border-white/[0.08] hover:border-[#FF5500]/60 transition-all duration-200 bg-[#161b22] flex flex-col justify-between shadow-[0_4px_20px_rgba(0,0,0,0.5)]"
                            >
                              <div className={`relative h-24 w-full overflow-hidden bg-gradient-to-br ${mapMeta.gradient}`}>
                                <div className="absolute inset-0 bg-black/40 backdrop-blur-[1px]" />
                                <div className="absolute top-2.5 left-3 right-3 flex items-center justify-between z-10">
                                  <div className="flex items-center gap-2">
                                    {mapMeta.icon && (
                                      <img src={mapMeta.icon} alt={mapMeta.name} className="w-6 h-6 object-contain drop-shadow" />
                                    )}
                                    <span className="text-sm font-black text-white uppercase tracking-wider drop-shadow">
                                      {mapMeta.name}
                                    </span>
                                  </div>
                                  <span className="px-2 py-0.5 rounded bg-black/70 backdrop-blur-sm border border-white/[0.15] text-[10px] text-gray-200 font-mono font-bold">
                                    {m.matches} Matches
                                  </span>
                                </div>
                              </div>
                              ...
                              <div className="p-3.5 pt-2 space-y-2.5">
                                <div className="grid grid-cols-3 gap-2 text-center">
                                  <div className="p-2 rounded bg-black/40 border border-white/[0.04]">
                                    <span className="text-[8px] text-gray-500 block uppercase font-bold">Win Rate</span>
                                    <span className={`text-xs font-black ${winRateNum >= 50 ? "text-emerald-400" : "text-gray-300"}`}>
                                      {m.winRate}%
                                    </span>
                                  </div>
                                  <div className="p-2 rounded bg-black/40 border border-white/[0.04]">
                                    <span className="text-[8px] text-gray-500 block uppercase font-bold">K/D</span>
                                    <span className="text-xs font-black text-white">{m.kdRatio}</span>
                                  </div>
                                  <div className="p-2 rounded bg-black/40 border border-white/[0.04]">
                                    <span className="text-[8px] text-gray-500 block uppercase font-bold">HS %</span>
                                    <span className="text-xs font-black text-cyan-400">{m.headshotPct}%</span>
                                  </div>
                                </div>

                                <div className="flex items-center justify-between text-[10px] text-gray-500 pt-1.5 border-t border-white/[0.05]">
                                  <span>Wins: <strong className="text-gray-300">{m.wins}</strong></span>
                                  <span className={`font-bold uppercase text-[9px] ${winRateNum >= 50 ? "text-emerald-400" : "text-gray-500"}`}>
                                    {winRateNum >= 50 ? "Positive Form" : "Sub-50%"}
                                  </span>
                                </div>
                              </div>
                            </div>
