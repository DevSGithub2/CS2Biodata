    ...
    <main className="min-h-screen bg-[#0d0f12] text-[#e1e7ed] font-sans antialiased pb-20 selection:bg-emerald-500 selection:text-black">
      ...
      <header className="h-14 border-b border-[#1b1f26] bg-[#11141a]/90 backdrop-blur-md px-6 flex items-center justify-between sticky top-0 z-50">
        <div className="flex items-center gap-3">
          <button
            onClick={() => router.push("/")}
            className="flex items-center gap-1 text-xs font-mono text-neutral-400 hover:text-white mr-2 transition"
          >
            <ArrowLeft className="w-3.5 h-3.5" /> Back
          </button>
          <div
            onClick={() => router.push("/")}
            className="flex items-center gap-1.5 font-black text-white text-base tracking-wider cursor-pointer"
          >
        ...
        {loading && !data && (
          <div className="flex flex-col items-center justify-center py-28 space-y-3">
            ...
            <p className="text-xs font-mono text-neutral-400 uppercase tracking-wider">
              Resolving player telemetry dossier...
            </p>
          </div>
        )}

        {error && (
          <div className="p-4 rounded-xl border border-red-500/30 bg-red-950/20 text-red-400 text-xs font-mono text-center">
            {error}
          </div>
        )}
          ...
          <>
              ...
              <div className="space-y-6">
                {!faceit?.registered || !fOverview ? (
                  ...
                  <>
                    ...
                    {/* Dedicated FACEIT Match History Feed */}
            ...
            {activeTab === "steam" && (
              <div className="space-y-6">
                ...
                {/* Valve Match History Sync Section */}
